...
def _filter_non_gates(ops: Dict[str, int]) -> Dict[str, int]:
    """Remove pseudo-operations that are not physical gates (barrier, measure, etc.)."""
...
def hw_gate_breakdown_per_seed(
    ...
    """
    Return a list of per-seed gate-breakdown rows after transpilation to basis_hw.
    Each row contains: depth, twoq_depth, counts for each basis gate, total, and other.
    """
    rows = []
    for sd in seeds:
        tqc = transpile(
        ...
        row = {
            "seed": int(sd),
            "depth": int(tqc.depth()),
            "twoq_depth": int(layer_count_of_gate(tqc, {"cx"})),  # this helper function is defined below
        }
        # gate counts within the selected basis
        for g in basis_hw:
            row[g] = int(ops.get(g, 0))

        # remaining gates, if transpilation produces gates outside the selected basis
...
def print_hw_gate_breakdown(
    ...
    # optionally print min/median/max for each gate in the selected basis
    def _mmm(key: str):
        xs = [r[key] for r in rows]
        return agg_min_med_max(xs)  # this helper function is defined below
...
def draw_and_save(qc: QuantumCircuit,
                  ...
                  dpi: int = 500,
                  scale: float = 0.85,
                  fold: int = 10**9,          # very large value to prevent line wrapping
                  fixed_inches=None,          # canvas size (W, H) in inches
                  tight: bool = True) -> str:
    """
    - tight=True  : tightly crop the drawing around the circuit content.
    - tight=False : preserve the full canvas, which is useful for equal-sized parts.
    - fixed_inches: force a fixed canvas size so all parts use identical dimensions.
    """
    try:
        ...
        if fixed_inches is not None:
            ...
            # left-align the content when the Matplotlib/Qiskit backend supports anchoring
            try:
                ax = fig.axes[0]
                ax.set_anchor("W")
...
def pad_part_to_cols(qc_part: QuantumCircuit, target_cols: int) -> QuantumCircuit:
    """Append barriers at the END to left-align the circuit and extend wires to the full canvas width."""
    ...
    for _ in range(target_cols - cur):
        out.barrier(all_q)   # each barrier approximately contributes one visual column
    return out
...
def pad_to_layers_end(qc: QuantumCircuit, target_layers: int) -> QuantumCircuit:
    """
    Pad at the END for left alignment by appending barriers until target_layers is reached.
    """
    cur = num_layers(qc)
    ...
    for _ in range(target_layers - cur):
        out.barrier(all_q)   # add one additional empty visual column/layer
    return out


def split_circuit_by_layers(qc: QuantumCircuit, cols_per_part: int = 48) -> List[QuantumCircuit]:
    """
    Split a circuit into multiple parts, each containing approximately cols_per_part DAG layers.
    Preserve the number of qubits/clbits and copy only the operations belonging to the corresponding layers.
    """
    ...
    # map qubits and clbits by index for robust reconstruction
...
def draw_and_save_segmented(qc: QuantumCircuit,
    ...
    # pad every part to the same number of columns so all wires have equal visual length
    parts = [pad_part_to_cols(p, cols_per_part) for p in parts]

    if len(parts) == 1:
        return draw_and_save(qc, base, dpi=dpi, scale=scale, tight=True)

    # pad every part to exactly cols_per_part layers for consistent left alignment
    parts = [pad_to_layers_end(p, cols_per_part) for p in parts]

    # Pass 1: determine the maximum canvas size across all parts
    sizes = []
    for p in parts:
    ...
    # Pass 2: save all parts using the same fixed canvas size
    k = len(parts)
...
if __name__ == "__main__":
    ...
    # maximum number of gate columns per drawn line (called fold in Qiskit)
    fold = 45  # use 16 for 16 columns, 20 for 20 columns, etc.

    # force all parts to use the same canvas so LaTeX does not enlarge the final part
    # fixed_size = (40, 3.2)  # (width_inch, height_inch); adjust for the desired qubit count and visual density

    dpi = 500
    SCALE = 1

    # transpiler seeds used to obtain min/median/max values for robustness
    seeds = [1, 2, 0, 4, 3]
